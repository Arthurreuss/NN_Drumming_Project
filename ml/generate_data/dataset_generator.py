import os
import pandas as pd
from ml.generate_data.create_matrix import CreateMatrix


class DatasetGenerator:
    def __init__(self, verbose=True, dataset_path="datasets/e-gmd-v1.0.0", dataset_config=None):
    ...
    def create_samples(self):
        ...
        for file_name in self.midi_file_names[:3]:
            if not os.path.exists(os.path.join(self.dataset_path, file_name)):
                ...
                continue
            data_point = self.data_csv[self.data_csv['midi_filename'] == file_name].iloc[0]
            bpm = data_point['bpm']
            beat_type = data_point['beat_type']
            style = data_point['style']
            file_path = os.path.join(self.dataset_path, file_name)
            matrix_gen = CreateMatrix(quantization=self.quantization, beats_per_bar=self.beats_per_bar, normalize_velocity=self.normalize_velocity)
    ...
    def save_samples(self, save_path="datasets/preprocessed_datasets/processed_samples.csv"):
